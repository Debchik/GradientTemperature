...
class TheoryScreen(ResponsiveScreen):
    def __init__(self, app):
        ...
        self.pdf_paths = {
            "rus": Path("_internal/theory/theory_ru.pdf"),
            "eng": Path("_internal/theory/theory_en.pdf"),
        }
    ...
    def _load_pdf_pages(self) -> None:
        ...
        path = self.pdf_paths.get(self.lang_code, self.pdf_paths["eng"])

        if fitz is None:
            self.status_lines = self._status_message("missing_lib", str(path))
            return

        if not path.exists():
            self.status_lines = self._status_message("missing_file", str(path))
            return
        ...
        try:
            ...
            for page in doc:
                ...
                data = pix.tobytes()
                surface = pygame.image.fromstring(data, (pix.width, pix.height), mode)
