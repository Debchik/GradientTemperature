...
class ParamSlider:
    ...
    def draw_check(self, params: dict) -> None:
        ...
        params[self.name_par] = self.dec_round(value)
    ...
    def _draw_labels(self, value: float) -> None:
        ...
        value_text = f"{self.dec_round(value)}"
        value_surface = self.value_font.render(value_text, True, (27, 32, 42))
        value_rect = value_surface.get_rect()
        value_rect.top = self.card_rect.top + self.padding
        value_rect.right = self.card_rect.right - self.padding
        self.screen.blit(value_surface, value_rect)

    # ------------------------------------------------------------------ API
    def getValue(self) -> float:
        return self.dec_round(self.slider.get_value())
...
class _SliderImpl:
    ...
    def get_value(self) -> float:
        val_range = max(1, self.track.width)
        button_val = self.button_rect.centerx - self.track.left
        ratio = button_val / val_range
